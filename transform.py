...
def blur(image, kernel_size):
    ...
    neighbor_range = kernel_size // 2 # how many neighbors to one side we need to look at (ie for a kernel of 3, this value should be 1)
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                # Going to use a naive implementation of iteratingg through each neighbor
                # and summing
    ...
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                ...
                for x_i in range(max(0, x-neighbor_range), min(x_pixels - 1,x+neighbor_range) + 1):
                    for y_i in range(max(0, y-neighbor_range), min(y_pixels - 1,y+neighbor_range) + 1):
                        # Need to find which value of the kernel this corresponds to
...
if __name__ == '__main__':
    ...
    # combine these and make an edge detection filter
    sobel_x_kernel = np.array([
        [1, 2, 1],
        [0, 0, 0],
        [-1, -2, -1]
